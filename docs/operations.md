...
`zkit_doctor` は、`ZKIT` / `ZDOTDIR` / `ZKIT_PRIVATE`、必要ファイル、代表コマンド、private repo、`rc.d` 候補、未定義変数リスクの簡易スキャンを行います。警告が出た場合は、該当パスや rc.d の内容を確認してください。
...
5. `~/.zkit/bin/zkit_doctor` で大きな警告が出ないか
6. `ZKIT_DEBUG=true zsh -l` で読み込まれるか
...
5. `zkit_doctor` と `ZKIT_DEBUG=true zsh -l` で確認する
6. PATH の重複や順序は `type -a` で確認する
